...
background-image: url(img/struggle.png)
background-size: contain

???

Another note, for some of you beginners, this might all seem really foreign to you. 

But stick with it, ask questions whenever you get lost, practice, and I promise it will come to you
...
R stores it as days since Jan 1, 1970
...
Every function contains one or more arguments
  + The parameters provided to a function to perform that operation
  
The first function, has one argument

The second function has more than one argument
  + The first argument is x which should be an object again
  + The second argument is na.rm and has a default input of FALSE

Arguments can also have defaults.
  + In this case, the default is False which means if there are NAs in your variable, a mean will not be calculated. 
  + If you change it to true, it means you want to calculate a mean, even if there are NAs in the variable. 
