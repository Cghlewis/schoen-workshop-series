...
cols: These are the variables you want to pivot longer. These are your repeating variables. So you can see here that scores repeat across time. So the variables I would choose for this argument are time1 and time2.

...

names_to: You will provide a name for the new categorical variable to store your old variable names. So when you pivot longer, your variable names will become a new categorical column. You can name this column whatever you want. I chose the name "time". Whatever you choose, this name needs to be in quotation marks when you add it to your argument.

values_to: You will provide a name for the new variable storing your values. When you pivot longer your values will be combined into this one column. Again, you can name this column whatever you want. I chose the name "score". This name needs to be in quotation marks when you add it to your argument.
...
cols: The cols argument doesn't change. This is still the variables you want to pivot longer.

names_to: The names_to argument is now two pieces of information, because there are two pieces of information in our variable name now (item and time)
...
cols: Is the same. It is still the columns we want to pivot longer. So in this example, we would want to pivot everything except for teacher id because these are all repeating variables.

names_to: is now two pieces of information, and these two pieces of information are separated by a comma

First you will provide a name for the categorical piece of your variable name to be made into a new variable. So in our example, the categorical pieces of our variable name are these years. So I would name this piece year. Again, I could name this anything I want, I just need to put it in quotation marks.

Second you will use the term .value, verbatim, in this argument. And what this does is it makes new columns based off of our item names

When you provide these two pieces of information for this argument, you need to provide them in the order in which they appear. So if the time component is first, list that first, if the time component is second, list that second
...
So again, let's look at this in actual code
...
For names_to, I would use that .value indicator to say, grab all of my item names and make new columns from those and I would name by second categorical piece, whatever I want. In this case I am going to name it year.

Remember I am giving these arguments in the order in which they appear in my variable names (item then year).

And last, for names_sep, I am going to say, split my variable names based on an underscore. This is how it knows what to grab for the item component and what to grab for the year component.
...
Again I think it's easiest to see how these arguments work through a visual

So here we have our long data with repeating participants and we want to transform this to a wide dataset

Our first argument in the pivot_wider function is names_from, which is, which column contains our new variable names (this is typically your categorical variable or time variable). In our example here, we would choose the time column.

Our second argument is values_from, which is, which column has the values we want to pivot into wide format. In our example here, we would choose the score column.

Note that we no longer need to use quotes for these argument because we are no longer naming variables we are referring to existing variables. 
...
So here is an example of pivoting a long dataset to wide.

In this example our names_from argument would be year. These are the values we are going to append to our variable names when we pivot wider. You can see how these years get appended to the item names when we pivot wider.

For values_from we are choosing all of the variables that repeat to pivot wider.

And for names_sep, in this example, I chose an underscore to separate the year and item name in our new variable names but you could choose whatever separator you want.
...
Truly the only way to get better at R is to force yourself to use it. It will be difficult to force yourself to do this because you will struggle for a while and you will want to fall back on the methods you are already comfortable with. However, if you just keep pushing through, at some point it will really start to click and it's a great feeling when it does. I tried to learn R off and on from like 2011 to 2018 and every time it got hard, I went back to using programs I was more comfortable with like SPSS and Stata. But eventually I decided to close out all of those programs and force myself to solely code in R and it didn't take long after that for me to start feeling more proficient. So just try to stick with it.
