...
We will need to either change all of our forward slashes to back slashes or add double back slashes to escape our backslash
...
So instead of typing that full path out, I can add one dot to indicate that my path should start at my current root directory
...
If you type here() it will show you your current working directory
...
This one is a lot more complicated to use, but I think it's a really helpful one to show you because as we know, there are many times when we need to concatenate time to our variable names when we merge data over time and this is the most efficient way to do that
...
A transposed version of print. Allows you to see the number of rows and columns as well as the class of your variables. Much more simplified look at your data
...
data %>%
  filter(character variable == "some name")
...
data %>%
  select(-c(variable5, variable6, variable7))
...
|conditional operator to regroup/recode a variable|dplyr|case_when|
...
If our date variable had values like "03-22-2022" then we could use `mdy()`
...
data %>%
  mutate(new variable =
           case_when(
             logical condition ~ a value,
             TRUE ~ another value
           ))

```

* some logical argument = a logical condition, usually comparing a variable to a value or another variable

* `~` = "then replace with" 

* a value = character, numeric, NA, date value, or an existing variable
...
* value = a value to replace NA with
]
...
* `~` = as a function of
* `.` = The dot means, refer to the variables referenced earlier for where to replace the NAs
...
replace_na: I'm not sure if you remember me talking a while back ago, how sometimes a certain district would send me a file of ISS counts for students and when a student had no ISS referrals, they would leave the cell blank, this is a way for me to take care of that
