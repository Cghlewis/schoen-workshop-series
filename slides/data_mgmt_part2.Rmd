...
In tools such as Qualtrics, RedCap, etc.
...
Of course these paper instruments have to be converted into an electronic format for analysis
...
There may be other ways to link IDs preemptively, like getting class rosters and adding all students to your participant tracking database and assigning IDs and then build those IDs into your instruments

But if there are no other options, and you just can't link name and ID before you collect, my only recommendation is that as soon as those forms come back to you with names, you first check if those participants already exist in your participant database (so you don't assign them a separate ID). If they do, you connect these forms to their existing ID. If they don't already exist, you assign those participants new IDs.
...
Here you have 6 students who consented
...
Don't do any of these things in any other file

This is the probably the most important rule
...
Say I'm the project coordinator, Every day when a paper forms comes to me, I'm going to check for missing data, duplicate data and out of range data and if everything looks ok, I'm going to track the form as complete
...
You still want to do those things (like comment your code)

As opposed to the slight meandering that sometimes happens during data cleaning where there is a lot of trial and error
...
Some of these come from experience working in your field and from working with lots of different kinds of data, others you actively collect information on to gain an understanding of
...
In terms of data cleaning, having data literacy provides you the intuition to make better decisions in your data cleaning processes

Remember last week we talked about how humans are fallible. We need to learn to anticipate mistakes, rather than assume our data will come to us error free. 

We also need to assume that we make mistakes in our data cleaning process as well. We hope we don't! But we also need to check that our transformations worked correctly.

Including the types of variables you work with (numeric, integer, categorical, string, logical).
...
In addition to coming from experience,
...
This has happened to me in the past and it created several errors when I tried to rename Q12 to gender but Q12 isn't gender anymore in the survey. Q12 is now race because of some sort of reordering in the instrument.
...
Or did we follow best practices and restrict all of our data entry fields in our data entry tool to only our allowable values

If anyone has done much work in Excel, you'll know that Excel tends to love to change values to dates that shouldn't be dates
...
This doesn't happen with just genetic data, I have seen this kind of bad formatting happen from various Pearson assessments that are exported as txt files but sometimes people would open those in Excel before sending to me and non-date values with dashes in them were formatted into dates.
...
Check to see how many cases failed and passed each test

Check mark means the test ran
...
We will be just getting to know our way around R
Getting comfortable with the interface and learning what we can do with R
